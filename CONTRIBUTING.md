...
For the optional OAuth / HTTP MCP components, use Node.js 24 LTS and install both independent lockfiles before their integration tests:
...
These installs do not change the Core's dependency model or configure a service. Include negative authorization, refresh replay, concurrency, process-restart, and existing Core regressions when changing the authentication boundary. Keep real client secrets, MFA material, identity mappings and auth databases outside the checkout. The [OAuth guide](docs/chatgpt-web-oauth-v0.1/README.md) separates local tests from public deployment and real-client acceptance.
...
The publication checker and fixed-version Gitleaks wrapper complement each other; neither proves complete absence of private facts. Missing scanner/dependencies must be reported, not treated as a pass. Follow the [publication policy](docs/publication-policy.md), including its opt-in commit hook and remote coverage limits, and review new text manually. Do not attach real conversations, memory exports, database copies, credentials, or deployment logs. Use synthetic test vectors and redact incidental environment details.
