...
export async function callTool(name, args = {}, env = process.env) {
  ...
  if (runtimeEnv.MNEMURON_HANDOFF_ENABLED === 'false' && HANDOFF_NEW_TOOLS.has(name)) {
    throw Object.assign(new Error('New handoff operations are disabled.'), {errorCode:'HANDOFF_DISABLED'});
  }
...
const HANDOFF_NEW_TOOLS = new Set(['mnemuron_preview_resume','mnemuron_confirm_resume',
  'mnemuron_preview_project_bootstrap','mnemuron_confirm_project_bootstrap','mnemuron_preview_task_bootstrap',
  'mnemuron_confirm_task_bootstrap','mnemuron_preview_reconciliation','mnemuron_confirm_reconciliation']);
export function toolsForRuntime(env = process.env) {
  const runtime = loadRuntimeEnv(env);
  // take_pending_resume is intentionally retained while an existing delivery drains.
  return runtime.MNEMURON_HANDOFF_ENABLED === 'false' ? TOOLS.filter(tool=>!HANDOFF_NEW_TOOLS.has(tool.name)) : TOOLS;
}

export async function listRuntimeTools(env = process.env) {
  const runtime = loadRuntimeEnv(env), local = toolsForRuntime(runtime);
  if (runtimeMode(runtime) !== 'remote') return local;
  let capabilities;
  try { capabilities = await remoteRequest(runtime,'GET','/v1/capabilities'); }
  catch (error) { if (error.statusCode === 404) return local; throw error; }
  if (capabilities.modules?.handoff?.enabled !== false) return local;
  return local.filter(tool=>!HANDOFF_NEW_TOOLS.has(tool.name)
    && (capabilities.modules.handoff.state === 'draining' || tool.name !== 'mnemuron_take_pending_resume'));
}
