...
import { RESPONSE_LIMIT, decodeResponse, protocolError, flushQueue, queueItems, queueState, queueSummary, validateAcceptance } from './sync-protocol.mjs';
import {
  loadRuntimeEnv,
...
// Maintenance-only opt-in. Ordinary hooks never retry unresolved acceptance mismatches.
export async function reconcileEventOutbox(eventIds, env = process.env) {
  if(!Array.isArray(eventIds) || !eventIds.length || eventIds.length>100
    || eventIds.some(id=>typeof id!=='string' || !/^[A-Za-z0-9][A-Za-z0-9_.:-]{0,127}$/.test(id))) throw protocolError('INVALID_RECONCILIATION_TARGET');
  return flushKind('event',env,eventIds);
}
...
async function flushKind(kind,env,reconcileEventIds) {
  const runtimeEnv=loadRuntimeEnv(env),root=resolveDataDir(runtimeEnv);
  const allItems=queueItems(path.join(root,queueDirectories[kind]),kind);
  const items=allItems.filter(item=>!reconcileEventIds || (reconcileEventIds.includes(path.basename(item.filePath,'.json'))
    && queueState(item).state==='blocked_reconciliation' && queueState(item).last_error_code==='RECEIPT_MISMATCH'));
  return flushQueue(items,{
    ...
    predecessors:[...Object.entries(queueDirectories).filter(([other])=>other!==kind).flatMap(([other,dir])=>queueItems(path.join(root,dir),other)),
      ...(reconcileEventIds?allItems.filter(item=>!items.includes(item)):[])],
