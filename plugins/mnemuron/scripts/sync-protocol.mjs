...
export async function flushQueue(items, {root,credential,send,accepted=()=>{},quarantine=null,predecessors=[],now=()=>Date.now(),random=Math.random,afterAccepted=()=>{},maxElapsedMs=15000,reconcileEventIds=[]}) {
  if(!Array.isArray(reconcileEventIds) || reconcileEventIds.length>100 || reconcileEventIds.some(id=>typeof id!=='string' || !/^[A-Za-z0-9][A-Za-z0-9_.:-]{0,127}$/.test(id))) throw protocolError('INVALID_RECONCILIATION_TARGET');
  const reconciliationTargets=new Set(reconcileEventIds);
  ...
  for(const item of items) {
    ...
    try {
      ...
      const fail=error=>{
        ...
        if(reconciliation) {
          atomicState(reconciliation.file,{...reconciliation.record,status:'blocked',error_code:state.last_error_code});
          state.last_reconciliation_error_code=state.last_error_code;
          state.state='blocked_reconciliation';state.last_error_code='RECEIPT_MISMATCH';state.next_retry_at=null;
        }
      ...
      if(predecessors.some(other=>other.filePath!==item.filePath && other.lane===item.lane && other.time<item.time && existsSync(other.filePath))) {
        if(!['quarantined','blocked_protocol','blocked_reconciliation'].includes(state.state)) atomicState(item.filePath+'.state',{...state,state:'blocked_gap',last_error_code:'PENDING_PREDECESSOR'});
      ...
      const canReconcile=item.kind==='event' && state.state==='blocked_reconciliation' && state.last_error_code==='RECEIPT_MISMATCH'
        && state.envelope_hash===item.envelope_hash && state.credential_fingerprint===credentialId
        && marker?.file===item.filePath && marker.state==='blocked_reconciliation'
        && reconciliationTargets.has(item.payload?.event?.event_id);
      if((['quarantined','blocked_protocol','blocked_reconciliation'].includes(state.state) && !canReconcile)
      ...
      try {
        if(canReconcile) {
          const directory=path.join(root,'sync-reconciliation');mkdirSync(directory,{recursive:true,mode:0o700});
          reconciliation={file:path.join(directory,randomUUID()+'.json'),record:{event_id:item.payload.event.event_id,
            envelope_hash:item.envelope_hash,previous_state:{...state},status:'attempting',at:new Date(now()).toISOString()}};
          atomicState(reconciliation.file,reconciliation.record);
        }
        attempts++;
        ...
        if(reconciliation) atomicState(reconciliation.file,{...reconciliation.record,status:'accepted',
          accepted_event_ids:response.accepted_event_ids,inserted:response.inserted,duplicate:response.duplicate,accepted_at:new Date(now()).toISOString()});
