...
import {MemorySources} from './memory/sources.mjs';
...
function resolverRequest(payload, { requireQuery = false, allowProjectId = false } = {}) {
...
export class MnemuronStore {
  constructor(databasePath, options = {}) {
    ...
    this.runtime = memoryRuntime(options.memoryConfig);
    this.storagePolicy = storageDoctor(privateStoragePaths(options.memoryConfig, this.databasePath, options.memoryConfigPath), {
      ...this.runtime, sourceRoots:options.sourceRoots,
    });
    ...
    try {
      ...
      this.memoryJobs = new MemoryJobs(this,{leaseMs:jobConfig.lease_ms,concurrency:jobConfig.concurrency,batchSize:jobConfig.batch_size});
      const providerOptions={synthetic:this.runtime.deploymentMode==='test' && this.runtime.syntheticData};
      this.organizer=this.runtime.organizerConfigured?new Organizer(this.memoryConfig.providers.organizer,providerOptions):null;
      this.embedder=this.runtime.embedderConfigured?new Embedder(this.memoryConfig.providers.embedder,providerOptions):null;
  ...
  deriveStructuredMemories(auth, checkpoint, events) {
    if (!this.runtime.memory || !this.runtime.captureExtraction) return {status:"disabled",extracted:0,created:0,existing:0,memories:[]};
    const sources = events.map(event => this.db.prepare("SELECT * FROM events WHERE user_id=? AND event_id=?").get(auth.user_id,event.event_id)).filter(Boolean);
    return this.memoryService.derive(auth, sources, checkpoint);
  }
  ...
  appendEvents(auth, input) {
    ...
    const {checkpoints,structured_memories} = dispatchCapture({
      handoffEnabled:this.handoffPolicy.enabled(),
      memoryEnabled:this.runtime.memory && this.runtime.captureExtraction,
      processHandoff:() => checkpointTriggers.map((eventId) => {
      try {
      ...
      processMemory:() => {
        try {
          const sources = events.map(event=>this.db.prepare("SELECT * FROM events WHERE user_id=? AND event_id=?").get(auth.user_id,event.event_id)).filter(Boolean);
          return this.memoryService.derive(auth,sources);
        } catch (error) {
          this.audit({auth,action:"memory.capture",outcome:"failed",metadata:{error_code:error.errorCode || "MEMORY_PROCESSING_FAILED"}});
          return {status:"failed",error_code:error.errorCode || "MEMORY_PROCESSING_FAILED"};
        }
      },
    });
  ...
  retractMemory(auth, memoryId, payload = {}) {
    ...
    this.revisions.record(this.db.prepare("SELECT * FROM memories WHERE memory_id=?").get(memoryId),"retracted");
    this.audit({
      auth,
  ...
  previewProjectContext(auth, payload) {
    ...
    const preview = {
      ...
      resolution,
      read_capabilities: { task_field_details: true },
  ...
  recordInjectionEvent(auth, resumeId, payload) {
    this.requireScope(auth, "resume:confirm");
    this.handoffPolicy.allowReceipt(auth,resumeId,payload,"injection");
    assertIdentifier(resumeId, "resume_id");
  ...
  recordDeliveryReceipt(auth, resumeId, payload) {
    this.requireScope(auth, "resume:confirm");
    this.handoffPolicy.allowReceipt(auth,resumeId,payload,"receipt");
    assertIdentifier(resumeId, "resume_id");
    if (!payload || typeof payload !== "object") {
  ...
  status(auth) {
    ...
    return {
      ...
      modules: {memory:{enabled:this.runtime.memory,state:this.runtime.memory?"ready":"disabled"},handoff:this.handoffPolicy.status(auth.user_id)},
      memory_runtime: {version:this.runtime.version,legacy_compatibility:this.runtime.legacy,migration_notice:this.runtime.notice || null,
        organizer:this.runtime.organizerConfigured?"configured_unverified":"not_configured",embedder:this.runtime.embedderConfigured?"configured_unverified":"not_configured",
        vector_index:this.vectorIndex?(this.vectorIndex.state().active?"ready":"unavailable"):"disabled",job_worker:this.runtime.workerConfigured?"configured_separate_process":"disabled"},
    ...
    try {
      return this.memoryTransaction(()=>{
        const result=this.db.prepare(`UPDATE events SET content=NULL,raw_payload_json=NULL,expired_at=? WHERE event_id IN (
          SELECT event_id FROM events WHERE expires_at<=? AND expired_at IS NULL ${auth ? 'AND user_id=?' : ''}
          AND NOT EXISTS (SELECT 1 FROM memory_source_pins p WHERE p.user_id=events.user_id AND p.event_id=events.event_id) ORDER BY expires_at,event_id LIMIT ?)`)
          .run(timestamp,timestamp,...(auth ? [auth.user_id] : []),batchSize);
        const remaining=this.db.prepare(`SELECT count(*) n FROM events WHERE expires_at<=? AND expired_at IS NULL ${auth ? 'AND user_id=?' : ''}
          AND NOT EXISTS (SELECT 1 FROM memory_source_pins p WHERE p.user_id=events.user_id AND p.event_id=events.event_id)`).get(timestamp,...(auth ? [auth.user_id] : [])).n;
