...
export function createMnemuronApp({
  ...
  const store = new MnemuronStore(databasePath, { defaultRetentionDays, memoryConfig, memoryConfigPath });
    ...
    try {
      ...
      if (request.method === 'GET' && pathname === '/v1/capabilities') {
        ...
        return sendJson(response,200,{read_only:true,production_ready:false,
          modules:{memory:{enabled:store.runtime.memory},handoff:store.handoffPolicy.status(auth.user_id)}});
      }
      ...
      if (request.method === "POST" && pathname === "/v1/memories/query") {
        const body = await readJson(request, maxBodyBytes);
        responseStatus = 200;
        return sendJson(response, 200, await store.searchMemories(auth, body));
      }
      if(request.method==='POST' && pathname==='/v1/memory-summaries/query'){
        const body=await readJson(request,maxBodyBytes);responseStatus=200;return sendJson(response,200,store.memorySummaries(auth,body));
      }
      if(request.method==='POST' && pathname==='/v1/memory-source-manifests/query'){
        const body=await readJson(request,maxBodyBytes);responseStatus=200;return sendJson(response,200,store.memorySources.manifest(auth,body));
      }
      const rawMemorySource=routeMatch(pathname,/^\/v1\/memory-sources\/([^/]+)$/);
      if(request.method==='GET' && rawMemorySource){
        const pagination={};for(const field of ['offset','limit'])if(url.searchParams.has(field))pagination[field]=Number(url.searchParams.get(field));
        responseStatus=200;return sendJson(response,200,store.memorySources.content(auth,rawMemorySource[0],pagination));
      }
