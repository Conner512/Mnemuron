...
export function consoleRead(store,auth,view,params={}) {
  ...
  const allowed=view==='memories'?['offset','limit','query']:view==='summary'?['summary_id','revision','cursor']:[];
  ...
  switch(view) {
    ...
    case 'memories': {
      if(params.query) {
        ...
        return store.searchMemories(auth,{query:params.query,limit:20,mode:'lexical'});
      }
      ...
      const rows=db.prepare('SELECT memory_id,content,memory_type,status,created_at FROM memories WHERE user_id=? ORDER BY created_at DESC,memory_id LIMIT ? OFFSET ?').all(user,limit+1,offset);
    ...
    case 'summaries':return {read_only:true,
      categories:db.prepare('SELECT category,COUNT(DISTINCT memory_id) count FROM memory_annotations WHERE user_id=? GROUP BY category').all(user),
    ...
    case 'jobs':return {read_only:true,jobs:db.prepare('SELECT job_id,job_type,state,total,processed,attempt_count,last_error_code,created_at,updated_at FROM memory_jobs WHERE user_id=? ORDER BY created_at DESC LIMIT 100').all(user),operations:'blocked_policy'};
    case 'connections':return {read_only:true,connections:db.prepare('SELECT credential_id,label,device_id,agent_id,agent_instance_id,created_at,last_used_at,revoked_at FROM credentials WHERE user_id=? ORDER BY created_at DESC LIMIT 100').all(user),operations:'blocked_policy'};
    case 'audit':return {read_only:true,entries:db.prepare('SELECT audit_id,action,target_type,target_id,outcome,created_at FROM audit_events WHERE user_id=? ORDER BY created_at DESC LIMIT 100').all(user)};
